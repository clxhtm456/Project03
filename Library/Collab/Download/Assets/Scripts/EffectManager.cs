...
public class EffectManager : Singleton<EffectManager> {
    ...
    public GameObject MakeUIEffect(string _name, Vector3 _position, Quaternion _qurt)
    {
        ResetList();
        GameObject particle = particleList.Find(item => item.name == _name && item.activeInHierarchy == false);
        if (particle != null)
        {
            ...
            particle.transform.rotation = _qurt;
            return particle;
        }
        GameObject _object = FindEffect(_name);
        if (_object != null)
        {
            ...
            newList.transform.rotation = _qurt;
            particleList.Add(newList);
            return newList;
        }
        else
    ...
    public GameObject MakeEffect(string _name,Vector3 _position,Quaternion _qurt)
    {
        ResetList();
        GameObject particle = particleList.Find(item => item.name == _name && item.activeInHierarchy == false);
        if (particle != null)
        ...
        if (_object != null)
        {
            GameObject newList = Instantiate(_object, _position, _qurt);
            newList.name = _object.name;
            particleList.Add(newList);
            return newList;
        }
