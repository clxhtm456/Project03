...
public class SpellCard : MonoBehaviour {
    ...
    public void ShowSpellDetail()
    {
        if (detail != null)
            return;
        detail = EffectManager.instance.MakeUIEffect("SpellDetail", new Vector3(-414.0f, 96.2f, 0.0f));
        ...
        detail.transform.Find("Frame").GetComponent<Image>().color = spell.spellCaster.state.frameColor;
        if (spell.spellIcon != null)
            detail.transform.Find("Icon").GetComponent<Image>().sprite = spell.spellIcon;
    }
	...
	void Update () {
        ...
        if (transform.parent != parentUI.transform)
        {
            if (EffectManager.IsGamePause())
                transform.parent = parentUI.transform;
            else
            {
